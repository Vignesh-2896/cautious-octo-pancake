...
class Interests extends Component {
    ...
    addForms =  (e) => {
        ...
        const NewBox = (
            <div key = {nextInterestId} data-key = {nextInterestId} className = {`interestForm_${nextInterestId}`} style = {divStyle}>
                <InputBox name = {`interest_name_${nextInterestId}`} placeholder = "Enter Interest" />
                <ButtonBox buttonInfo = "Delete" buttonAction = {this.deleteForms} />
            </div>
        )

        this.setState({
            interestList: this.state.interestList.concat(NewBox),
        })
    }  
