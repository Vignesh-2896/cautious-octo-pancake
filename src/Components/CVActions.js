...
class CVActions extends Component {
  ...
  async sampleData(e) {
    // Function to input sample data into all the fields.
    ...
    for (const element of response) {
      // Iterating with all the sample data.
      if (!currentInputType || element === "") {
        if (element === "") {
          currentInputType = "";
          continue;
        } else if (element === "Work Description")
          currentInputType = "SameBox";
        else currentInputType = "NextBox";
      } else {
        if (currentInputType === "NextBox") {
          inputList[inputCtr].value = element;
          inputCtr++;
        } else {
          textArea[0].value += element + "\n"; // For the textarea box, multiple lines will be added to same field.
        }
      }
    }
  ...
  clearCV(e) {
    ...
    alert(
      "Manually added sections can be removed using their respective Delete buttons."
    );
  }

  async fetchData(e) {
    // Function to generate PDF based on form inputs.
