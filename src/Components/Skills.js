...
class Skills extends Component {
    ...
    addForms =  (e) => {
        ...
        const NewBox = (
            <div key = {nextSkillId} data-key = {nextSkillId} className = {`skillForm_${nextSkillId}`} style = {divStyle}>
                <InputBox name = {`skill_name_${nextSkillId}`} placeholder = "Enter Skill" />
                <ButtonBox buttonInfo = "Delete" buttonAction = {this.deleteForms} />
            </div>
        )

        this.setState({
            skillList: this.state.skillList.concat(NewBox),
        })
    }  
