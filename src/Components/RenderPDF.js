...
function RenderPDF() {
  ...
  let generalInput = Array.from(
    document
      .getElementsByClassName("generalInfo")[0]
      .getElementsByTagName("input")
  );
  let skillInput = Array.from(
    document.getElementsByClassName("skills")[0].getElementsByTagName("input")
  );
  let educationInput = Array.from(
    document
      .getElementsByClassName("education")[0]
      .getElementsByTagName("input")
  );
  let experienceInput = Array.from(
    document.getElementsByClassName("work")[0].getElementsByTagName("input")
  );
  let textAreaInput = Array.from(
    document.getElementsByClassName("work")[0].getElementsByTagName("textarea")
  );
  let interestInput = Array.from(
    document
      .getElementsByClassName("interests")[0]
      .getElementsByTagName("input")
  );
  ...
  generalSection.push(
    <View>
      <Text style={{ fontSize: "22px" }} key={generalInput[0].name}>
      ...
      <View style={{ display: "flex", flexDirection: "row" }}>
        <Link
          key={generalInput[1].name}
          src={`mailto:${generalInput[1].value}`}
        >
          <Image style={styles.soMoIcon} src={MailSVG} />
        </Link>
        <Link key={generalInput[4].name} src={generalInput[4].value}>
          <Image style={styles.soMoIcon} src={LinkedinSVG} />
        </Link>
        <Link key={generalInput[5].name} src={generalInput[5].value}>
          <Image style={styles.soMoIcon} src={GithubSVG} />
        </Link>
      </View>
    </View>
  );
  ...
  educationInput.forEach(function (item) {
    if (item.name.startsWith("cv_ins_"))
      educationUnits.push(
        <Text style={styles.boldOnly} key={item.name}>
          {item.value}
        </Text>
      );
    else if (item.name.startsWith("cv_degree_"))
      educationUnits.push(
        <Text style={{ color: "#333" }} key={item.name}>
          {item.value}
        </Text>
      );
  ...
  for (let i = 0; i < educationUnits.length; i += 6) {
    educationSection.push(
      <View style={styles.viewSection}>
        <Text>
          {educationUnits[i]} - {educationUnits[i + 1]}
        </Text>
        <Text>
          {educationUnits[i + 3]} - {educationUnits[i + 4]}
        </Text>
        <Text>
          Degree in {educationUnits[i + 2]} with an overall aggregate of{" "}
          {educationUnits[i + 5]}
        </Text>
      </View>
    );
  ...
  textAreaInput.forEach(function (item) {
    // Transforming each line in each textarea box to include a bullet point.
    let textAreaUnits = item.value.split("\n").map((arrItem, index) => (
      <Text key={`${item.name}_${index}`}>
        <Image style={styles.bullet} src={bulletPoint} />
        {arrItem}
      </Text>
    ));
  ...
  experienceInput.forEach(function (item) {
    if (item.name.startsWith("wrk_cpy_name_"))
      experienceUnits.push(
        <Text style={styles.boldOnly} key={item.name}>
          {item.value}
        </Text>
      );
    else if (item.name.startsWith("wrk_designation_"))
      experienceUnits.push(
        <Text style={{ color: "#333" }} key={item.name}>
          {item.value}
        </Text>
  ...
  for (let i = 0; i < experienceUnits.length; i += 5) {
    ...
    for (let j = startFrom; j < startFrom + textAreaSize[i % 5]; j++)
      textAreaSection.push(tempList[j]);

    experienceSection.push(
      <View style={styles.viewSection}>
        <Text>
          {experienceUnits[i]} - {experienceUnits[i + 1]}
        </Text>
        <Text>
          {experienceUnits[i + 2]} - {experienceUnits[i + 3]}
        </Text>
  ...
  interestInput.forEach(function (item) {
    interestUnits.push(
      <View style={styles.viewSection}>
        <Text key={item.name}>
          <Image style={styles.bullet} src={bulletPoint} /> {item.value}
        </Text>
      </View>
    );
  });
  ...
  const MyDoc = (
    <Document>
      <Page size="A4" style={styles.page}>
        <View style={styles.view}>{generalSection}</View>
